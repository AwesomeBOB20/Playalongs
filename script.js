document.addEventListener('DOMContentLoaded', function () {
  ...
  let currentOriginalTempo       = null;   // tempo anchor
  let userIsAdjustingTempo       = false;  // guard while dragging slider
  let randomizeBusy              = false;  // guard re-entrancy for randomize
  ...
  if (audio) {
    audio.loop = false;
    // iOS tempo sounds better when pitch isn’t preserved
  ...
  // --- Ended behavior (randomize-friendly) ---
  if (audio) {
  ...
  function updateSliderBackground(slider, color1, color2) {
    ...
    slider.style.background =
        `linear-gradient(to right, ${color1} 0%, ${color1} ${percentage}%, ${color2} ${percentage}%, ${color2} 100%)`;
}
  ...
  function pickRandomTempo() {
    ...
    try {
      ...
      minTempo = Math.max(minTempo, parseInt(tempoSlider.min, 10));
      maxTempo = Math.min(maxTempo, parseInt(tempoSlider.max, 10));
